...
rec {
  latestVersion =
    attrs:
    (builtins.head (
      builtins.sort (a: b: (builtins.compareVersions a.version b.version) == 1) (
  ...
  selectSystem =
    ...
    if system == "aarch64-darwin" then
      alt (lib.findFirst ({ host, ... }: (match "arm64-apple-darwin.*" host) != null) null systems) (
        selectSystem "x86_64-darwin" systems
      )
    else if system == "x86_64-darwin" then
      alt (lib.findFirst ({ host, ... }: (match "x86_64-apple-darwin.*" host) != null) null systems) (
